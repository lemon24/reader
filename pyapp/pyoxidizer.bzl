...
def make_exe(dist):
    ...
    # ###
    python_config.run_module = "reader.__main__"
    ...
    exclude = ['yaml._yaml', 'markupsafe._speedups']

    resources = exe.pip_install(["..[search,cli,app]"])
    resources_filtered = []

    for resource in resources:
        if resource.name in exclude:
            continue

        if type(resource) == 'PythonPackageResource' and resource.package == 'reader._app':
            resource.add_location = 'filesystem-relative:.'

        #print(resource.package, resource.name)

        resources_filtered.append(resource)

    exe.add_python_resources(resources_filtered)
